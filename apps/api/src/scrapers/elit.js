// Elit B2B Scraper
// Elit is a Slovenian/European automotive parts supplier
...
async function login(page, credentials) {
  try {
    console.log('[elit] logging in...');

    if (credentials.sessionCookie) {
      console.log('[elit] reusing session cookie');
      return true;
    }

    const baseUrl = 'https://www.elit.si';
    ...
    if (!form) {
      console.log('[elit] login form not found');
      return false;
    }
    ...
    if (!emailInput || !passInput) {
      console.log('[elit] login fields not found');
      return false;
    }
    ...
    if (errorEl) {
      ...
      if (errorText && errorText.trim().length > 0) {
        console.log('[elit] login failed:', errorText.trim());
        return false;
      }
    }

    console.log('[elit] login successful');
    return true;
  } catch (err) {
    console.log('[elit] login error:', err.message);
    return false;
  }
}
...
async function search(page, partNumber) {
  try {
    console.log('[elit] searching for:', partNumber);
    ...
    if (priceNet === null) {
      console.log('[elit] product not found for:', partNumber);
      return [];
    }

    console.log('[elit] found product:', productName, 'price:', priceNet);
    return [{
    ...
    console.log('[elit] search error:', err.message);
    return [];
  }
}
